...
def test_data_collection(logger):
    ...
    collected_gall = data_collect.crawl_dcinside(logger)
    collected_youtube  = data_collect.crawl_youtube(logger)
    logger.info('{} gall data collected.'.format(len(collected_gall)))
    logger.info('{} youtube data collected.'.format(len(collected_youtube)))
    logger.info('---------------------------------')
    data_collect.save_csv()
